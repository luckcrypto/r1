(function() {
  ...
  const dateHolder   = document.querySelector(".date-today-12word");
  const timeHolder   = document.querySelector(".timestamp-12word");
  const ticketBody   = document.getElementById("numbers-12word");
  const quickpick    = document.getElementById("quick-pick-12word");
  const resetButton  = document.getElementById("reset-ticket-12word");
  ...
  function addOneLine() {
    if (lineCount >= MAX_LINES) {
      alert("You've reached the maximum of 3 phrases! Click RESET to start over.");
      return;
    }
