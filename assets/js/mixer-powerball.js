// Combined Powerball main + Powerball drum simulation
function startLotteryDrum(canvasId) {
    ...
    const gravity = 0.68;
    const blowerPower = 2.8;      // slightly stronger to mix 95 balls better
    const blowerRadius = 58;
    const friction = 0.9985;      // tiny bit less friction for more motion
    ...
    // Ball color by type
    function getBallColor(isPowerball, number) {
        return isPowerball ? '#FF2323' : '#ffffff';
    }

    class Ball {
        constructor(x, y, velX, velY, size, number, isPowerball = false) {
            this.x = x;
            this.y = y;
            this.velX = velX;
            this.velY = velY;
            ...
            this.baseColor = getBallColor(isPowerball, number);
        }
        ...
        draw() {
            ...
            // Number text
            ctx.fillStyle = this.isPowerball ? '#111111' : '#000000';
    ...
    // Red Powerballs (a bit more scattered / higher)
    for (let i = 1; i <= 26; i++) {
        const size = 12;  // slightly smaller to distinguish
        let x, y, dist;
        do {
            const angle = Math.PI + random(-Math.PI / 2.8, Math.PI / 2.8);
            const radius = random(spreadRadius * 0.3, spreadRadius * 1.1);
            x = centerX + radius * Math.cos(angle);
            y = centerY + radius * Math.sin(angle) + bottomOffset - random(30, 80); // higher start
            dist = Math.hypot(x - centerX, y - centerY);
    ...
    function drawDrum() {
        ...
        // Drum background, glass, rim, highlights (unchanged from your code)
