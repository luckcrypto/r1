(function() {
  // DOM element references - Oz Lotto
  const dateHolder = document.querySelector(".date-today-oz");
  const timeHolder = document.querySelector(".timestamp-oz");
  const ticketBody = document.getElementById("numbers-oz");
  const quickpick = document.getElementById("quick-pick-oz");
  const resetButton = document.getElementById("reset-ticket-oz");
  ...
  function addOneLine() {
    ...
    copyBtn.addEventListener("click", () => {
      ...
      // For Oz Lotto: just the 7 main numbers (space separated)
  ...
  function generatePick() {
    ...
    // 7 unique numbers from 1–47 (current Oz Lotto main numbers range)
    while (pick.length < 7) {
      const value = randomNumber(47, 1);
