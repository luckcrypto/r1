(function() {
  ...
  const dateHolder   = document.querySelector(".date-today-24word");
  const timeHolder   = document.querySelector(".timestamp-24word");
  const ticketBody   = document.getElementById("numbers-24word");
  const quickpick    = document.getElementById("quick-pick-24word");
  const resetButton  = document.getElementById("reset-ticket-24word");

  const MAX_LINES = 2;
  let lineCount = 0;

  // Full official BIP-39 English word list (2048 words)
  const wordList = [
    ...
    'clever','click','client','cliff','climb','clinic','clip','clock','clog','cloth',
    ...
    'dirt','disagree','discover','disease','dish','disorder','display','distance','divert','divide',
    ...
    'evidence','evil','evoke','evolve','exact','example','excess','exchange','exclude','excuse',
    ...
    'primary','priority','prison','private','prize','problem','process','produce','profit','program',
    ...
    'resemble','resist','resource','response','result','retire','retreat','reunion','reveal','review',
    ...
    'similar','simple','since','sing','siren','sister','situated','six','size','skate',
    'sketch','ski','skill','skin','skirt','skull','slab','slam','slash','sleep',
    ...
    'spice','spider','spike','spin','spirit','split','spoiled','sponsor','spoon','sport',
    ...
    'swear','sweet','swift','swim','swing','switch','sword','symbol','symptom','syndrome',
    ...
    'truck','true','truly','trumpet','trust','truth','tube','tuition','tumble','tuna',
    ...
    'width','wife','wiggle','wild','will','wimpy','win','window','wine','wing',
  ...
  function addOneLine() {
    if (lineCount >= MAX_LINES) {
      alert("You've reached the maximum of two 24-word phrases! Click RESET to start over.");
      return;
    }
    ...
    textContainer.className = "phrase-container";
    textContainer.textContent = phrase;
  ...
  function resetTicket() {
    // Instead of clearing elements → refresh the whole page
    window.location.reload();
  }

  function generatePick() {
    const shuffled = wordList.slice();
